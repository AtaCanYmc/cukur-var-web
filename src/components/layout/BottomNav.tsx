...
const NavItem = ({ icon: Icon, active, onClick, label }: NavItemProps) => {
    return (
        <button
            onClick={onClick}
            className={`flex flex-col items-center justify-center transition-all duration-300 ${active ? 'text-orange-500 scale-110' : 'text-slate-500'
                }`}
        >
...
export const BottomNav: React.FC = () => {
    ...
    return (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[92%] max-w-[400px] z-50">
            <div
                className="flex items-center justify-between px-8 py-3 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border border-slate-200 dark:border-slate-800 shadow-[0_20px_50px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.3)] rounded-[32px] transition-colors duration-300">
